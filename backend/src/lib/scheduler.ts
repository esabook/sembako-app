...
function cekRingkasanHarian(): Alert[] {
	...
	const [row] = db
		...
		.from(penjualan)
		.where(and(sql`date(${penjualan.tanggal}) = ${hariIni}`, ne(penjualan.status, 'void')))
		.all();
...
function jalankanCek(cfg: typeof notifikasi_config.$inferSelect): void {
	...
	switch (cfg.jenis) {
		case 'stok_habis':
			...
			break;
		case 'stok_kritis':
			alerts = cekStokKritis();
			break;
		case 'barang_kadaluarsa':
			alerts = cekKadaluarsa(threshold);
			break;
		case 'hutang_jatuh_tempo':
			alerts = cekHutangJatuhTempo(threshold);
			break;
		case 'piutang_macet':
			alerts = cekPiutangMacet(threshold);
			break;
		case 'ringkasan_harian':
			alerts = cekRingkasanHarian();
			break;
		case 'ringkasan_mingguan':
			alerts = cekRingkasanMingguan();
			break;
	}
...
// ── Grace hapus toko — flag 'deleted' saat hapus_terjadwal lewat ────────────
// Tanpa hapus fisik: data tetap, login dikunci (gating di auth.ts). Purge fisik
// = aksi manual admin. Idempotent: setelah flip, hapus_terjadwal di-null.
function prosesHapusTerjadwal(): void {
	const now = new Date().toISOString();
	const jatuh = db
		.select({ id: toko.id, nama: toko.nama })
		.from(toko)
		.where(and(isNotNull(toko.hapus_terjadwal), lte(toko.hapus_terjadwal!, now)))
		.all();

	for (const t of jatuh) {
		db.update(toko)
			.set({ status_langganan: 'deleted', is_active: false, hapus_terjadwal: null })
			.where(eq(toko.id, t.id!))
			.run();
		console.log(`[scheduler] toko #${t.id} (${t.nama}) → deleted (grace habis)`);
	}
}
...
function tickScheduler(): void {
	try {
		prosesHapusTerjadwal();
	} catch (err) {
		console.error('[scheduler] error proses hapus terjadwal:', err);
	}
