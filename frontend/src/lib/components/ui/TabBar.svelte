...
<script lang="ts">
  ...
  function onDrop(targetKey: string) {
    if (!dragKey || dragKey === targetKey) { dragKey = null; dragOver = null; return }
  ...
  function ctxGeser(key: string, arah: -1 | 1) {
    ...
    if (newIdx < 0 || newIdx >= current.length) { closeCtx(); return }
...
<div
  style="display:flex; gap:.5rem; border-bottom:1px solid var(--border); margin-bottom:1rem;
...
>
  {#each orderedTabs() as t (t.key)}
    ...
    >
      <!-- Indikator drop -->
      {#if isDragTarget}
        <div style="position:absolute; left:0; top:0; bottom:0; width:2px; background:var(--accent); border-radius:2px"></div>
      {/if}
      ...
      <button
        ...
        style="display:inline-flex; align-items:center; gap:.3rem; padding:.5rem .75rem;
               ...
               color:{isActive ? 'var(--accent)' : 'var(--text-dim)'}; font-family:inherit; font-size:.8rem;
      ...
      {#if hoverTab === t.key || (ctx?.key === t.key)}
        <button
          onclick={(e) => { e.stopPropagation(); openCtx(e, t.key) }}
...
{#if ctx}
  ...
  <div
    ...
    role="menu"
  >
    <div style="padding:.35rem .85rem .2rem; font-size:.72rem; color:var(--text-dim); font-weight:600; text-transform:uppercase; letter-spacing:.04em">
      {label}
    </div>
    ...
    <button
      ...
      disabled={idx === 0}
      style="display:flex; align-items:center; gap:.5rem; width:100%; padding:.45rem .85rem;
             background:none; border:none; color:{idx === 0 ? 'var(--text-dim)' : 'var(--text)'}; cursor:{idx === 0 ? 'default' : 'pointer'}; text-align:left; font-family:inherit; font-size:.82rem; opacity:{idx === 0 ? 0.4 : 1}"
      onmouseenter={(e) => { if (idx > 0) (e.currentTarget as HTMLElement).style.background = 'var(--surface2)' }}
      onmouseleave={(e) => (e.currentTarget as HTMLElement).style.background = 'none'}
    >
      ← Geser kiri
    </button>
    ...
    <button
      ...
      style="display:flex; align-items:center; gap:.5rem; width:100%; padding:.45rem .85rem;
             background:none; border:none; color:{idx === ordered.length - 1 ? 'var(--text-dim)' : 'var(--text)'}; cursor:{idx === ordered.length - 1 ? 'default' : 'pointer'}; text-align:left; font-family:inherit; font-size:.82rem; opacity:{idx === ordered.length - 1 ? 0.4 : 1}"
      onmouseenter={(e) => { if (idx < ordered.length - 1) (e.currentTarget as HTMLElement).style.background = 'var(--surface2)' }}
      onmouseleave={(e) => (e.currentTarget as HTMLElement).style.background = 'none'}
    >
      → Geser kanan
    </button>
