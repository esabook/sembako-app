...
{#if open}
	...
	<div
		...
		onclick={tutup}
		onkeydown={(e) => {
			if (e.key === 'Escape') klikKiri();
		}}
	>
		<!-- Panel iOS -->
		<!-- svelte-ignore a11y_no_static_element_interactions -->
		<div class="panel" onclick={(e) => e.stopPropagation()} onkeydown={handleKeydown}>
			<!-- Teks -->
			<div class="ios-body">
			...
			<div class="ios-actions">
				<button
					bind:this={btnKiri}
					onclick={klikKiri}
					class="ios-btn py-3 text-sm"
					class:ios-focused={!fokusKanan}
					style="color:{warnaKiri ?? 'var(--accent)'}">{labelKiri}</button
				>
				...
				<button
					...
					class="ios-btn py-3 text-sm"
					class:ios-focused={fokusKanan}
					style="color:{warnaKanan ?? 'var(--accent)'}">{labelKanan}</button
				>
			</div>
		</div>
	</div>
