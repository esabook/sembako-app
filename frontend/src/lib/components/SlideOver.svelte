...
<!-- Panel -->
<div
	class="so-panel fixed right-0 bottom-0 left-0
         z-50 flex flex-col rounded-t-2xl border-t
         sm:inset-y-0 sm:right-0 sm:left-auto sm:rounded-none sm:border-t-0 sm:border-l"
	style="
...
>
	...
	<button
		type="button"
		class="resize-handle absolute inset-y-0 left-0 z-10 hidden w-2 cursor-ew-resize items-center sm:flex"
		class:active={wDragging}
		onmousedown={startWidthDrag}
		onkeydown={(e) => {
			if (e.key === 'ArrowLeft') {
				panelWidth = Math.min(panelWidth + 8, Math.round(window.innerWidth * 0.85));
				localStorage.setItem(KEY_W, String(panelWidth));
				e.preventDefault();
			}
			if (e.key === 'ArrowRight') {
				panelWidth = Math.max(panelWidth - 8, MIN_W);
				localStorage.setItem(KEY_W, String(panelWidth));
				e.preventDefault();
			}
		}}
	...
	>
		<div class="indicator ml-0.5 h-10 w-0.5 rounded-full" style="background:var(--accent)"></div>
	</button>

	<!-- Mobile drag handle (top) ───────────────────────────────────────────── -->
	<div
		class="flex shrink-0 cursor-ns-resize touch-none justify-center pt-3 pb-1 sm:hidden"
	...
	>
		<div
			class="h-1 w-10 rounded-full transition-colors"
			style="background:{hDragging ? 'var(--accent)' : 'var(--border)'}"
		></div>
	</div>
	...
	<!-- Header ─────────────────────────────────────────────────────────────── -->
	<div class="shrink-0 border-b" style="border-color:var(--border);padding:16px;padding-right:48px">
