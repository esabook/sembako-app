...
<div>
	<div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:1rem">
		...
		{#if tab === 'jurnal' || tab === 'kasbank'}
			<Button
				onclick={() => (tab === 'jurnal' ? store.bukaModalJurnal() : store.bukaTambahKasBank())}
			>
	...
	<div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:.6rem; margin-bottom:1rem">
		<div
			style="background:var(--surface); border:1px solid var(--border); border-radius:6px; padding:.65rem .9rem"
		>
			<div style="font-size:.65rem; color:var(--text-dim); margin-bottom:.2rem">
				HUTANG SUPPLIER
			</div>
			<div style="font-size:1rem; font-weight:700; color:var(--danger)">
				Rp {fmt(store.totalHutangBelum)}
			</div>
		</div>
		<div
			style="background:var(--surface); border:1px solid var(--border); border-radius:6px; padding:.65rem .9rem"
		>
			...
			<div style="font-size:1rem; font-weight:700; color:var(--warn)">
				Rp {fmt(store.totalPiutangBelum)}
			</div>
		</div>
		<div
			style="background:var(--surface); border:1px solid var(--border); border-radius:6px; padding:.65rem .9rem"
		>
			...
			<div style="font-size:1rem; font-weight:700; color:var(--info)">
				Rp {fmt(store.totalSaldo)}
			</div>
		</div>
	</div>
