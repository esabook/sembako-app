...
<SlideOver bind:open={store.formOpen} title={store.editRow ? 'Edit Acara' : 'Tambah Acara'}>
	<div class="space-y-4">
		<Input
			bind:value={store.fNamaAcara}
			label="Nama Acara *"
			placeholder="Pernikahan, Syukuran, dll"
		/>
		<Input
			bind:value={store.fNamaPenyelenggara}
		...
		<div>
			<label for="fh-estimasi" class="mb-1 block text-sm font-medium" style="color:var(--text-dim)"
				>Estimasi Tamu</label
			>
			<input
				id="fh-estimasi"
		...
		<div>
			<p class="mb-1 block text-sm font-medium" style="color:var(--text-dim)">Status</p>
			<div class="flex flex-wrap gap-2">
				{#each STATUS_LIST as s (s)}
					<button
						onclick={() => (store.fStatus = s)}
		...
		<div>
			<label for="fh-catatan" class="mb-1 block text-sm font-medium" style="color:var(--text-dim)"
				>Catatan</label
			>
			<textarea
				id="fh-catatan"
