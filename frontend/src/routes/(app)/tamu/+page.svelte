<script lang="ts">
  ...
  function bukaForm() {
    fNama = ''; fInstansi = ''; fKeperluan = ''
    fTanggal = new Date().toISOString().slice(0, 10)
    fJamMasuk = ''; fJamKeluar = ''; fKeterangan = ''
    error = ''; formOpen = true
  }

  async function simpan() {
    error = ''
    if (!fNama.trim()) { error = 'Nama tamu wajib'; return }
    if (!fKeperluan.trim()) { error = 'Keperluan wajib'; return }
    if (!fTanggal) { error = 'Tanggal wajib'; return }
    const r = await api.post('/tamu', {
      nama_tamu: fNama.trim(), instansi: fInstansi.trim() || undefined,
      keperluan: fKeperluan.trim(), tanggal: fTanggal,
      jam_masuk: fJamMasuk || undefined, jam_keluar: fJamKeluar || undefined,
      keterangan: fKeterangan.trim() || undefined,
    })
    if (!r.success) { error = (r as any).error; return }
    formOpen = false; muatTamu()
  }
  ...
  $effect(() => { dariBulan; muatTamu() })
</script>

<div class="flex flex-col gap-4">
  <div class="flex flex-wrap gap-2 items-center">
    <h1 class="text-base font-bold mr-2">Tamu Birokrasi</h1>
    <input type="month" bind:value={dariBulan}
    ...
    <button onclick={bukaForm}
      class="px-3 py-1 rounded text-sm font-bold ml-auto" style="background:var(--accent);color:var(--bg)">+ Catat Tamu</button>
  </div>
      ...
      <table class="min-w-full text-sm" style="border-collapse:collapse;min-width:500px">
        <thead>
          <tr style="background:var(--surface2)">
            ...
            <th class="px-3 py-2 text-left text-xs font-semibold" style="color:var(--text-dim)">Nama Tamu</th>
            <th class="px-3 py-2 text-left text-xs font-semibold hidden sm:table-cell" style="color:var(--text-dim)">Instansi</th>
            <th class="px-3 py-2 text-left text-xs font-semibold" style="color:var(--text-dim)">Keperluan</th>
            <th class="px-3 py-2 text-left text-xs font-semibold hidden sm:table-cell" style="color:var(--text-dim)">Jam</th>
        ...
        <tbody>
          {#each rows as row (row.id)}
            <tr class="border-t" style="border-color:var(--border)">
              ...
              <td class="px-3 py-2 text-xs hidden sm:table-cell" style="color:var(--text-dim)">{row.instansi ?? '—'}</td>
              <td class="px-3 py-2 text-sm">
                <div>{row.keperluan}</div>
                {#if row.keterangan}
                  <div class="text-xs mt-0.5" style="color:var(--text-dim)">{row.keterangan}</div>
                {/if}
              </td>
              ...
              <td class="px-3 py-2 text-right">
                <button onclick={() => hapus(row.id)} class="text-xs px-2 py-0.5 rounded" style="color:var(--danger)">Hapus</button>
              </td>
            </tr>
          {/each}
        </tbody>
...
<SlideOver bind:open={formOpen} title="Catat Tamu Birokrasi">
  ...
  <form onsubmit={(e) => { e.preventDefault(); simpan() }} class="flex flex-col gap-3 text-sm">
    <div class="flex flex-col gap-1">
      ...
      <input id="ft-nama" bind:value={fNama} required placeholder="Nama lengkap tamu"
    ...
    <div class="flex flex-col gap-1">
      ...
      <input id="ft-inst" bind:value={fInstansi} placeholder="mis. Dinas Perdagangan, Kelurahan"
        class="px-2 py-1 rounded border outline-none"
        style="background:var(--surface2);border-color:var(--border);color:var(--text)" />
    </div>
    <div class="flex flex-col gap-1">
      <label for="ft-kep" class="text-xs" style="color:var(--text-dim)">KEPERLUAN *</label>
      <input id="ft-kep" bind:value={fKeperluan} required placeholder="mis. Inspeksi BPOM, Perpanjang izin"
        class="px-2 py-1 rounded border outline-none"
        style="background:var(--surface2);border-color:var(--border);color:var(--text)" />
    </div>
    <div class="grid grid-cols-2 gap-3">
      <div class="flex flex-col gap-1 col-span-2">
        ...
        <input id="ft-tgl" type="date" bind:value={fTanggal} required
          class="px-2 py-1 rounded border outline-none"
          style="background:var(--surface2);border-color:var(--border);color:var(--text)" />
      </div>
      <div class="flex flex-col gap-1">
        <label for="ft-masuk" class="text-xs" style="color:var(--text-dim)">JAM MASUK</label>
        <input id="ft-masuk" type="time" bind:value={fJamMasuk}
          class="px-2 py-1 rounded border outline-none"
          style="background:var(--surface2);border-color:var(--border);color:var(--text)" />
      </div>
      <div class="flex flex-col gap-1">
        <label for="ft-keluar" class="text-xs" style="color:var(--text-dim)">JAM KELUAR</label>
        <input id="ft-keluar" type="time" bind:value={fJamKeluar}
          class="px-2 py-1 rounded border outline-none"
          style="background:var(--surface2);border-color:var(--border);color:var(--text)" />
      </div>
    </div>
    <div class="flex flex-col gap-1">
      ...
      <input id="ft-ket" bind:value={fKeterangan} placeholder="Catatan tambahan (opsional)"
        class="px-2 py-1 rounded border outline-none"
        style="background:var(--surface2);border-color:var(--border);color:var(--text)" />
    </div>
    {#if error}
    ...
    <div class="flex justify-end gap-2 mt-1">
      <button type="button" onclick={() => formOpen = false} class="px-3 py-1 rounded text-sm"
