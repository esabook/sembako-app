...
<div class="flex flex-wrap gap-2 items-end mb-2">
  <select bind:value={store.asetKategoriFilter}
  ...
  <select bind:value={store.asetKondisiFilter}
    class="border rounded px-2 py-1 text-sm" style="background:var(--surface);border-color:var(--border);color:var(--text)">
  ...
  <button onclick={() => store.bukaFormAset()}
        ...
        {#each store.asetRows as row (row.id)}
          <tr class="border-t" style="border-color:var(--border)">
            ...
            <td class="px-3 py-2 text-right whitespace-nowrap">
              <button onclick={() => store.bukaFormAset(row)} class="text-xs px-2 py-0.5 rounded mr-1"
                style="border:1px solid var(--border);color:var(--text-dim)">Edit</button>
              <button onclick={() => store.hapusAset(row.id, row.nama)} class="text-xs px-2 py-0.5 rounded"
