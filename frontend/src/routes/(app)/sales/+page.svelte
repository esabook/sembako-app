...
<script lang="ts">
  ...
  import { createKunjunganStore, createAgendaStore, createPipelineStore } from './sales.store.svelte.js'
  ...
  $effect(() => { if (tab === 'kunjungan') { kStore.bulan; kStore.status; kStore.muat() } })
  $effect(() => { if (tab === 'agenda') { aStore.bulan; aStore.status; aStore.muat() } })
  $effect(() => { if (tab === 'pipeline') { pStore.tahap; pStore.muat() } })
</script>
