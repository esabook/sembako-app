<script lang="ts">
	...
	$effect(() => {
		store.filterBulanGaji;
		store.muatPenggajian();
	});
</script>
			...
			<tr class="border-t" style="border-color:var(--border)">
				...
				{#if !hidden.has('hadir_kerja')}
					<td class="px-3 py-2 text-center" style="color:var(--text-dim)">
						...
						{#if item.tipe_gaji === 'harian'}<span class="ml-1 text-xs">(H)</span>{/if}
					</td>
				{/if}
				...
				{#if !hidden.has('tunjangan')}
					<td class="px-3 py-2 text-right" style="color:var(--accent)"
						>{item.tunjangan > 0 ? rp(item.tunjangan) : '-'}</td
					>
				{/if}
				{#if !hidden.has('potongan_kasbon')}
					<td class="px-3 py-2 text-right" style="color:var(--warn)"
						>{item.potongan_kasbon > 0 ? rp(item.potongan_kasbon) : '-'}</td
					>
				{/if}
				{#if !hidden.has('potongan_lain')}
					<td class="px-3 py-2 text-right" style="color:var(--danger)"
						>{item.potongan_lain > 0 ? rp(item.potongan_lain) : '-'}</td
					>
				{/if}
				...
				{#if !hidden.has('aksi')}
					<td class="px-3 py-2 text-right whitespace-nowrap">
						{#if item.status === 'draft'}
							<button
								onclick={() => store.bukaEditGaji(item)}
								class="mr-2 text-xs"
								style="color:var(--info)">Edit</button
							>
							<button
								onclick={() => store.updateStatusGaji(item.id, 'approved')}
								class="mr-2 text-xs"
								style="color:var(--accent)">Approve</button
							>
							<button
								onclick={() => store.hapusGaji(item.id)}
								class="text-xs"
								style="color:var(--danger)">Hapus</button
							>
						{:else if item.status === 'approved'}
							<button
								onclick={() => store.bukaBayar(item.id)}
								class="text-xs font-bold"
								style="color:var(--accent)">Tandai Dibayar</button
							>
						{:else}
...
{#if store.penggajianList.length > 0}
	<div
		class="flex justify-end rounded border px-3 py-2 text-sm font-bold"
		style="border-color:var(--border);background:var(--surface2)"
	>
		<span style="color:var(--text-dim)">Total Penggajian &nbsp;</span>
		<span style="color:var(--accent)"
			>{rp(store.penggajianList.reduce((s, r) => s + r.total_gaji, 0))}</span
		>
	</div>
{/if}
...
<SlideOver bind:open={store.modalGajiOpen} title="Edit Tunjangan & Potongan">
	{#snippet children()}
		<form
			onsubmit={(e) => {
				e.preventDefault();
				store.simpanEditGaji();
			}}
			class="flex flex-col gap-3 text-sm"
		>
			{#if store.editGaji}
				<p class="text-xs" style="color:var(--text-dim)">
					{store.editGaji.nama_karyawan} — {store.editGaji.periode_bulan}
				</p>
				<p class="text-xs">
					Gaji pokok: <strong>{rp(store.editGaji.gaji_pokok)}</strong> &nbsp; Potongan kasbon
					otomatis: <strong>{rp(store.editGaji.potongan_kasbon)}</strong>
				</p>
			{/if}
			<div class="grid grid-cols-2 gap-3">
				<div class="flex flex-col gap-1">
					...
					<input
						...
						type="number"
						min="0"
						bind:value={store.formGaji.tunjangan}
						placeholder="0"
				...
				<div class="flex flex-col gap-1">
					...
					<input
						...
						type="number"
						min="0"
						bind:value={store.formGaji.potongan_lain}
						placeholder="0"
						class="input input-bordered w-full text-sm"
					/>
				</div>
			...
			<div class="mt-1 flex justify-end gap-2">
				<button
					type="button"
					onclick={() => (store.modalGajiOpen = false)}
...
<SlideOver bind:open={store.modalBayarOpen} title="Tandai Gaji Dibayar">
	{#snippet children()}
		<div class="flex flex-col gap-3 text-sm">
			...
			<div class="mt-1 flex justify-end gap-2">
				<button
					onclick={() => (store.modalBayarOpen = false)}
					class="rounded px-3 py-1 text-sm"
					style="color:var(--text-dim)">Batal</button
				>
				<button
					onclick={store.konfirmasBayar}
					class="rounded px-3 py-1 text-sm font-bold"
