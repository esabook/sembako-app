<script lang="ts">
  ...
  $effect(() => { store.siKaryawanId; store.siBulan; store.siTipe; store.muatSI() })
</script>

<div class="flex flex-wrap gap-2 mb-3 items-end">
  <select bind:value={store.siKaryawanId}
  ...
  <input type="month" bind:value={store.siBulan}
    class="border rounded px-2 py-1 text-sm" style="background:var(--surface);border-color:var(--border);color:var(--text)">
  <select bind:value={store.siTipe}
    class="border rounded px-2 py-1 text-sm" style="background:var(--surface);border-color:var(--border);color:var(--text)">
  ...
  {#if isManager}
    <button onclick={() => store.bukaFormSI()}
      ...
      {#each store.siRows as row (row.id)}
        <tr class="border-t text-sm" style="border-color:var(--border)">
          ...
          <td class="py-2 text-right">
            {#if isManager}
              <button onclick={() => store.hapusSI(row.id)} class="text-xs px-2 py-0.5 rounded" style="color:var(--danger)">Hapus</button>
            {/if}
          </td>
        </tr>
      {/each}
...
<SlideOver bind:open={store.siFormOpen} title="Catat Sanksi / Insentif">
  {#snippet children()}
  <form onsubmit={(e) => { e.preventDefault(); store.simpanSI() }} class="flex flex-col gap-3 text-sm">
    <div class="flex flex-col gap-1">
      <label for="fsi-karyw" class="text-xs" style="color:var(--text-dim)">KARYAWAN *</label>
      <select id="fsi-karyw" bind:value={store.fSiKaryawanId} required
      ...
      <div class="flex gap-4">
        {#each ([['insentif','Insentif'],['sanksi','Sanksi']] as const) as [v, lbl] (v)}
          <label class="flex items-center gap-1.5 cursor-pointer">
            <input type="radio" bind:group={store.fSiTipe} value={v} class="accent-[var(--accent)]" />
    ...
    <div class="flex flex-col gap-1">
      ...
      <input id="fsi-jenis" bind:value={store.fSiJenis} required placeholder="mis. Bonus penjualan, Keterlambatan, ..."
        class="px-2 py-1 rounded border outline-none"
        style="background:var(--surface2);border-color:var(--border);color:var(--text)" />
    </div>
    <div class="grid grid-cols-2 gap-3">
      <div class="flex flex-col gap-1">
        ...
        <input id="fsi-jml" type="number" min="1" bind:value={store.fSiJumlah} required
          class="px-2 py-1 rounded border outline-none"
          style="background:var(--surface2);border-color:var(--border);color:var(--text)" />
      </div>
      <div class="flex flex-col gap-1">
        <label for="fsi-tgl" class="text-xs" style="color:var(--text-dim)">TANGGAL *</label>
        <input id="fsi-tgl" type="date" bind:value={store.fSiTanggal} required
          class="px-2 py-1 rounded border outline-none"
          style="background:var(--surface2);border-color:var(--border);color:var(--text)" />
      </div>
    </div>
    <div class="flex flex-col gap-1">
      ...
      <input id="fsi-bulan" type="month" bind:value={store.fSiBulan} required
        class="px-2 py-1 rounded border outline-none"
        style="background:var(--surface2);border-color:var(--border);color:var(--text)" />
    </div>
    <div class="flex flex-col gap-1">
      <label for="fsi-ket" class="text-xs" style="color:var(--text-dim)">KETERANGAN</label>
      <input id="fsi-ket" bind:value={store.fSiKet} placeholder="Opsional"
        class="px-2 py-1 rounded border outline-none"
        style="background:var(--surface2);border-color:var(--border);color:var(--text)" />
    </div>
    {#if store.siError}
    ...
    <div class="flex justify-end gap-2 mt-1">
      <button type="button" onclick={() => store.siFormOpen = false} class="px-3 py-1 rounded text-sm"
