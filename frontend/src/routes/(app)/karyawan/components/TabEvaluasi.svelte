...
<div class="mb-3 flex flex-wrap items-end gap-2">
	...
	{#if isManager}
		<button
			onclick={() => store.bukaEvalForm()}
			class="ml-auto rounded px-3 py-1 text-sm font-bold"
			style="background:var(--accent);color:var(--bg)">+ Nilai</button
		>
	{/if}
</div>
	...
	<table class="min-w-full text-sm">
		<thead
			><tr class="text-xs" style="color:var(--text-dim)">
				...
				<th class="py-2 pr-3 text-center">Nilai</th>
				<th class="py-2 pr-3 text-left">Catatan</th>
		...
		<tbody>
				...
				<tr class="border-t text-sm" style="border-color:var(--border)">
					...
					<td class="py-2 pr-3 text-center">
						<span
							...
							style="color:{row.nilai >= 4
								? 'var(--accent)'
								: row.nilai <= 2
									? 'var(--danger)'
									: 'var(--warn)'}">{'★'.repeat(row.nilai)}{'☆'.repeat(5 - row.nilai)}</span
						>
					</td>
			...
			{#if !store.evalRows.length}
				<tr
					><td colspan="6" class="py-6 text-center text-sm" style="color:var(--text-dim)"
...
<!-- ── Modal: Form Evaluasi ──────────────────────────────────────────────────── -->
<SlideOver
	bind:open={store.evalFormOpen}
	title={store.editEvalId ? 'Edit Evaluasi' : 'Tambah Evaluasi'}
>
	{#snippet children()}
		<form
			onsubmit={(e) => {
		...
		>
			...
			<div class="flex flex-col gap-1">
				<label for="fe-periode" class="text-xs" style="color:var(--text-dim)"
					>PERIODE (YYYY-MM) *</label
				>
				<input
					id="fe-periode"
					type="month"
					bind:value={store.fEvalPeriode}
					required
					class="rounded border px-2 py-1 outline-none"
					style="background:var(--surface2);border-color:var(--border);color:var(--text)"
				/>
			...
			<div class="flex flex-col gap-1">
				...
				<div class="flex gap-1">
					{#each [1, 2, 3, 4, 5] as n (n)}
						<button
							type="button"
							onclick={() => (store.fEvalNilai = String(n))}
					...
					{/each}
					<span class="ml-2 self-center text-sm" style="color:var(--text-dim)"
						>{store.fEvalNilai}/5</span
					>
				</div>
			</div>
			<div class="flex flex-col gap-1">
			...
			<div class="mt-1 flex justify-end gap-2">
				<button
					type="button"
					onclick={() => (store.evalFormOpen = false)}
