<script lang="ts">
	...
	const FIELDS: { key: FieldKey; label: string; required: boolean; desc: string }[] = [
		{ key: 'nama_barang', label: 'Nama Barang', required: true, desc: 'Nama lengkap produk' },
		{
			...
			label: 'Kode Barang',
			required: false,
			desc: 'Otomatis jika kosong (BRG-0001)'
		},
	...
	function parseFile(file: File) {
		...
		Papa.parse<ParsedRow>(file, {
			...
			complete(res) {
				...
				for (const h of csvHeaders) {
					const match = FIELDS.find(
						(f) =>
							f.key !== '__abaikan' &&
							(f.key === h.toLowerCase().replace(/\s+/g, '_') ||
			...
			error(err) {
				parseError = `Gagal membaca file: ${err.message}`;
			}
		});
	}
	...
	function buildMappedRows() {
		...
		mappedRows = csvRows.map((raw, i): MappedRow => {
			...
			if (get('kategori_nama'))
				warnings.push(`Kategori "${get('kategori_nama')}" akan dicek/dibuat`);
	...
	const filteredRows = $derived(() => {
		...
		if (valFilter === 'warning')
			return mappedRows.filter((r) => r.errors.length === 0 && r.warnings.length > 0);
		if (valFilter === 'valid')
			return mappedRows.filter((r) => r.errors.length === 0 && r.warnings.length === 0);
		return mappedRows;
	});
...
<!-- ── Layout ── -->
<div class="mx-auto max-w-4xl space-y-5 p-3 md:p-6">
	<!-- Header -->
	<div class="flex items-center gap-3">
		<a
			href="/gudang?tab=barang"
			class="rounded border px-2 py-1 text-sm"
			style="border-color:var(--border);color:var(--text-dim)">← Kembali</a
		>
		<h1 class="font-bold" style="color:var(--text)">Import Barang dari CSV</h1>
	</div>

	<!-- Step Indicator -->
	<div class="flex items-center gap-1">
		{#each [[1, 'Upload'], [2, 'Peta Kolom'], [3, 'Validasi'], [4, 'Pengaturan'], [5, 'Hasil']] as [number, string][] as [n, label] (n)}
			<div class="flex items-center gap-1">
				<div
					class="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full text-xs font-bold transition-colors"
					style={step === n
				...
				<span
					class="hidden text-xs whitespace-nowrap sm:inline"
					style="color:{step === n ? 'var(--text)' : 'var(--text-dim)'}">{label}</span
				>
			</div>
			{#if n < 5}
				<div class="mx-1 h-px flex-1" style="background:var(--border)"></div>
			{/if}
		{/each}
	</div>
	...
	{#if step === 1}
		<div class="space-y-4">
			...
			>
				<div>
					...
					<p class="mt-0.5 text-xs" style="color:var(--text-dim)">
						Download file contoh CSV yang sudah terformat
					</p>
				</div>
				<button
			...
			<div
				...
				style="border-color:{dragOver ? 'var(--accent)' : 'var(--border)'};background:{dragOver
			...
			>
				...
				<p class="mt-1 mb-4 text-xs" style="color:var(--text-dim)">atau</p>
				<label
					class="cursor-pointer rounded px-4 py-2 text-sm font-medium text-white"
				...
				{#if fileName}
					<p class="mt-3 font-mono text-xs" style="color:var(--accent)">
						📎 {fileName} — {csvRows.length} baris terdeteksi
					</p>
				{/if}
			</div>
						...
						<tbody>
							{#each csvRows.slice(0, 5) as row, i (i)}
								<tr style="border-bottom:1px solid var(--border)">
									{#each csvHeaders as h (h)}
										<td
											class="max-w-32 truncate px-3 py-1.5 whitespace-nowrap"
											style="color:var(--text)">{row[h] ?? ''}</td
										>
									{/each}
								</tr>
							{/each}
					...
					{#each csvHeaders as h (h)}
						<div class="flex items-center gap-3 px-3 py-2">
							<div class="min-w-0 flex-1">
								<p class="truncate text-sm font-medium" style="color:var(--text)">{h}</p>
	...
	{:else if step === 3}
		<div class="space-y-4">
			<!-- Summary chips -->
			<div class="flex flex-wrap gap-2">
				{#each [['semua', `Semua (${mappedRows.length})`, 'var(--text-dim)', 'var(--surface2)'], ['valid', `Valid (${totalValid - totalWarning})`, '#059669', '#d1fae5'], ['warning', `Peringatan (${totalWarning})`, '#d97706', '#fef3c7'], ['error', `Error (${totalError})`, '#dc2626', '#fee2e2']] as [k, label, color, bg] (k)}
					<button
						onclick={() => (valFilter = k as typeof valFilter)}
						class="rounded-full border px-3 py-1 text-xs font-medium transition-opacity"
						style="color:{color};background:{valFilter === k
							? bg
							: 'var(--surface2)'};border-color:{color};opacity:{valFilter === k ? 1 : 0.7}"
					>
						{label}
					</button>
				{/each}
				...
				<table class="min-w-full text-xs">
					<thead>
						<tr style="border-bottom:1px solid var(--border)">
							...
							<th
								...
								style="color:var(--text-dim)">Kode</th
							>
							<th
							...
							<th
								class="hidden px-3 py-2 text-left font-semibold md:table-cell"
								style="color:var(--text-dim)">Satuan</th
							>
							<th class="px-3 py-2 text-left font-semibold" style="color:var(--text-dim)"
								>Harga Jual</th
							>
					...
					<tbody>
						{#each filteredRows() as row (row.index)}
							<tr
								class="border-b"
								style="border-color:var(--border);background:{row.errors.length
							...
							>
								...
								<td class="px-3 py-2 font-medium" style="color:var(--text)">
									{#if row.nama_barang}{row.nama_barang}{:else}<em style="color:var(--danger)"
											>kosong</em
										>{/if}
								</td>
								<td class="hidden px-3 py-2 font-mono sm:table-cell" style="color:var(--text-dim)">
									{#if row.kode_barang}{row.kode_barang}{:else}<em style="color:var(--text-dim)"
											>auto</em
										>{/if}
								</td>
								<td class="hidden px-3 py-2 md:table-cell" style="color:var(--text-dim)"
									>{row.kategori_nama || '—'}</td
								>
								<td class="hidden px-3 py-2 md:table-cell" style="color:var(--text-dim)"
									>{row.satuan_nama || '—'}</td
								>
	...
	{:else if step === 4}
		<div class="space-y-4">
			...
			>
				...
				{#each [['skip', 'Lewati baris ini (biarkan data lama)'], ['update', 'Update data yang ada dengan data baru'], ['generate', 'Buat kode baru otomatis (BRG-xxxx)']] as [val, label] (val)}
			...
			>
				...
				<label class="flex cursor-pointer items-center gap-2">
					...
					<span class="text-sm" style="color:var(--text)"
						>Buat <strong>kategori</strong> baru otomatis jika belum ada</span
					>
				</label>
				<label class="flex cursor-pointer items-center gap-2">
					<input type="checkbox" bind:checked={satuanAuto} class="accent-green-500" />
					<span class="text-sm" style="color:var(--text)"
						>Buat <strong>satuan</strong> baru otomatis jika belum ada</span
					>
				</label>
			</div>
			...
			<div class="rounded border p-4" style="background:var(--surface2);border-color:var(--border)">
				<p class="text-sm" style="color:var(--text)">
					...
					{#if totalError > 0}, melewati <strong style="color:var(--danger)"
							>{totalError} baris error</strong
						>{/if}
				</p>
			</div>
			...
			{:else if result}
				...
				<div class="grid grid-cols-3 gap-3">
					...
					>
						...
						<p class="mt-1 text-xs" style="color:var(--text-dim)">Berhasil</p>
					</div>
					<div
						class="rounded-lg border p-4 text-center"
						style="background:var(--surface);border-color:var(--border)"
					>
						...
						<p class="mt-1 text-xs" style="color:var(--text-dim)">Di-skip</p>
					</div>
					<div
						class="rounded-lg border p-4 text-center"
						style="background:var(--surface);border-color:var(--border)"
					>
						<p
							...
							style="color:{result.gagal.length ? '#dc2626' : 'var(--text-dim)'}"
						>
							{result.gagal.length}
						</p>
						<p class="mt-1 text-xs" style="color:var(--text-dim)">Gagal</p>
					</div>
				</div>
					...
					>
						{#if result.kategori_dibuat.length}
							<p style="color:var(--text-dim)">
								Kategori dibuat: <span style="color:var(--text)"
									>{result.kategori_dibuat.join(', ')}</span
								>
							</p>
						{/if}
						{#if result.satuan_dibuat.length}
							<p class="mt-1" style="color:var(--text-dim)">
								Satuan dibuat: <span style="color:var(--text)"
									>{result.satuan_dibuat.join(', ')}</span
								>
							</p>
						{/if}
					</div>
				...
				{#if result.gagal.length}
					<div class="rounded border" style="border-color:var(--border)">
						...
						>
							<p class="text-xs font-semibold" style="color:var(--danger)">
								Baris Gagal ({result.gagal.length})
							</p>
							<button
