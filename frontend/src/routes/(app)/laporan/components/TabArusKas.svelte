...
<ChartKartu kosong={!store.arusKas} pesanKosong="Pilih periode lalu klik Tampilkan.">
	{#if store.arusKas}
		...
		<!-- Ringkasan saldo -->
		<div
			style="display:grid; grid-template-columns:repeat(3,1fr); gap:.75rem; margin-bottom:1.5rem"
		>
			{#each [['Saldo Awal', arusKas.saldo_awal, 'var(--text)'], ['Net Periode', arusKas.net, arusKas.net >= 0 ? 'var(--accent)' : 'var(--danger)'], ['Saldo Akhir', arusKas.saldo_akhir, arusKas.saldo_akhir >= 0 ? 'var(--accent)' : 'var(--danger)']] as [label, val, warna] (label)}
				<div
					style="background:var(--surface); border:1px solid var(--border); border-radius:6px; padding:.75rem 1rem"
				>
				...
				<thead>
					<tr>
						{#each ['Akun', 'Saldo Awal', 'Masuk', 'Keluar', 'Saldo Akhir'] as h (h)}
							<th
								style="padding:.4rem .5rem; text-align:{h === 'Akun'
									? 'left'
									: 'right'}; color:var(--text-dim); font-size:.72rem; font-weight:600">{h}</th
							>
						{/each}
					</tr>
				</thead>
				<tbody>
					{#each arusKas.per_akun as akun (akun.id)}
						<tr style="border-bottom:1px solid var(--border)">
							...
							<td style="padding:.4rem .5rem; text-align:right; color:var(--text-dim)"
								>Rp {fmt(akun.saldo_awal)}</td
							>
							<td style="padding:.4rem .5rem; text-align:right; color:var(--accent)"
								>+Rp {fmt(akun.masuk)}</td
							>
							<td style="padding:.4rem .5rem; text-align:right; color:var(--danger)"
								>−Rp {fmt(akun.keluar)}</td
							>
							<td
								style="padding:.4rem .5rem; text-align:right; font-weight:700; color:{akun.saldo_akhir >=
								0
									? 'var(--accent)'
									: 'var(--danger)'}"
							>
					...
					<tr style="font-weight:700; border-top:2px solid var(--border)">
						...
						<td style="padding:.5rem .5rem; text-align:right; color:var(--text-dim)"
							>Rp {fmt(arusKas.saldo_awal)}</td
						>
						<td style="padding:.5rem .5rem; text-align:right; color:var(--accent)"
							>+Rp {fmt(arusKas.total_masuk)}</td
						>
						<td style="padding:.5rem .5rem; text-align:right; color:var(--danger)"
							>−Rp {fmt(arusKas.total_keluar)}</td
						>
						<td
							style="padding:.5rem .5rem; text-align:right; color:{arusKas.saldo_akhir >= 0
								? 'var(--accent)'
								: 'var(--danger)'}"
						>
				...
				<table style="width:100%; border-collapse:collapse; font-size:.83rem">
					<thead>
						<tr>
							{#each ['Kategori', 'Masuk', 'Keluar'] as h (h)}
								<th
									style="padding:.35rem .5rem; text-align:{h === 'Kategori'
										? 'left'
										: 'right'}; color:var(--text-dim); font-size:.72rem; font-weight:600">{h}</th
								>
							{/each}
					...
					<tbody>
						{#each Object.entries(arusKas.per_kategori).sort((a, b) => b[1].masuk + b[1].keluar - (a[1].masuk + a[1].keluar)) as [kat, val] (kat)}
							<tr style="border-bottom:1px solid var(--border)">
								<td style="padding:.35rem .5rem; color:var(--text); text-transform:capitalize"
									>{kat.replace(/_/g, ' ')}</td
								>
								<td
									style="padding:.35rem .5rem; text-align:right; color:{val.masuk > 0
										? 'var(--accent)'
										: 'var(--text-dim)'}"
								>
								...
								<td
									style="padding:.35rem .5rem; text-align:right; color:{val.keluar > 0
										? 'var(--danger)'
										: 'var(--text-dim)'}"
								>
