...
<div style="display:flex; gap:.75rem; align-items:center; margin-bottom:1rem; flex-wrap:wrap">
  <div style="display:flex; gap:.4rem; align-items:center">
    ...
    <input id="dari" type="date" bind:value={store.periode.dari}
  ...
  <div style="display:flex; gap:.4rem; align-items:center">
    ...
    <input id="sampai" type="date" bind:value={store.periode.sampai}
      style="padding:.35rem .6rem; background:var(--surface2); border:1px solid var(--border); border-radius:4px; color:var(--text); font-family:inherit; font-size:.82rem" />
  </div>
  <button
  ...
  >Tampilkan</button>
  {#each [
    { label: 'Hari ini', fn: () => { const t = new Date().toLocaleDateString('sv-SE'); store.periode = { dari: t, sampai: t }; store.muat('laba-rugi') } },
    { label: 'Minggu ini', fn: () => { const now = new Date(); const mon = new Date(now); mon.setDate(now.getDate() - now.getDay() + 1); const sun = new Date(mon); sun.setDate(mon.getDate() + 6); store.periode = { dari: mon.toLocaleDateString('sv-SE'), sampai: sun.toLocaleDateString('sv-SE') }; store.muat('laba-rugi') } },
    { label: 'Bulan ini', fn: () => { store.periode = store.defaultPeriode(); store.muat('laba-rugi') } },
  ] as s (s.label)}
    <button
      onclick={s.fn}
