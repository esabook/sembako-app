...
<div style="display:flex; gap:.75rem; align-items:center; margin-bottom:1rem; flex-wrap:wrap">
	...
	{#each [{ label: 'Hari ini', fn: () => {
				store.neracaTanggal = new Date().toLocaleDateString('sv-SE');
				store.muatNeraca();
			} }, { label: 'Akhir bulan lalu', fn: () => {
				const d = new Date();
				d.setDate(0);
				store.neracaTanggal = d.toLocaleDateString('sv-SE');
				store.muatNeraca();
			} }] as s (s.label)}
		<button
			onclick={s.fn}
	...
	<span style="font-size:.72rem; color:var(--text-dim)"
		>* nilai persediaan stok = kondisi saat ini</span
	>
</div>
...
{#if store.aging}
	...
	<div style="max-width:760px">
		...
		{#each [{ title: 'Piutang Pelanggan', buckets: aging.piutang, total: aging.total_piutang, prefix: 'p' }, { title: 'Hutang Supplier', buckets: aging.hutang, total: aging.total_hutang, prefix: 'h' }] as section (section.prefix)}
			<div style="margin-bottom:1.75rem">
				<div
					style="font-size:.75rem; font-weight:700; color:var(--text-dim); text-transform:uppercase; letter-spacing:.05em; margin-bottom:.75rem; padding-bottom:.35rem; border-bottom:1px solid var(--border)"
				>
				...
				{#each section.buckets as bucket, i (bucket.label)}
					...
					{#if bucket.jumlah > 0}
						<div
							style="margin-bottom:.5rem; border:1px solid var(--border); border-radius:6px; overflow:hidden; background:var(--surface)"
						>
							<button
							...
							>
								<div style="display:flex; align-items:center; gap:.6rem">
									<span
										style="width:8px; height:8px; border-radius:50%; background:{warna}; display:inline-block; flex-shrink:0"
									></span>
								...
								<div style="display:flex; align-items:center; gap:.75rem">
									<span style="font-size:.9rem; font-weight:700; color:{warna}"
										>Rp {fmt(bucket.total)}</span
									>
									<span style="font-size:.8rem; color:var(--text-dim)"
										>{store.agingExpanded[key] ? '▲' : '▼'}</span
									>
								</div>
							</button>
									...
									<table style="width:100%; font-size:.78rem; border-collapse:collapse">
										<thead>
											<tr style="background:var(--surface2)">
												<th
													style="padding:.4rem .85rem; text-align:left; color:var(--text-dim); font-weight:600"
													>Nama</th
												>
												<th
												...
												>
												<th
													style="padding:.4rem .85rem; text-align:right; color:var(--text-dim); font-weight:600"
													>Sisa</th
												>
											</tr>
										</thead>
										<tbody>
											{#each bucket.items as item, j (j)}
												<tr style="border-top:1px solid var(--border)">
													...
													<td style="padding:.35rem .5rem; text-align:right; color:var(--text-dim);"
														>{item.jatuh_tempo || '—'}</td
													>
													<td
														style="padding:.35rem .5rem; text-align:right; color:{item.hari > 0
															? warna
															: 'var(--text-dim)'}; "
													>
														{item.hari < 0
															? `${Math.abs(item.hari)}h lagi`
															: item.hari === 0
																? 'Hari ini'
																: `${item.hari}h`}
													</td>
													<td
														style="padding:.35rem .85rem; text-align:right; color:var(--text); font-weight:600"
														>Rp {fmt(item.sisa)}</td
													>
												</tr>
											{/each}
					...
					{:else}
						<div
							style="display:flex; justify-content:space-between; align-items:center; padding:.45rem .85rem; border-radius:4px; margin-bottom:.35rem; background:var(--surface)"
						>
							<div style="display:flex; align-items:center; gap:.6rem">
								<span
									style="width:8px; height:8px; border-radius:50%; background:var(--border); display:inline-block"
								></span>
