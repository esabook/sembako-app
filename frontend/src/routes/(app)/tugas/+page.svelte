<script lang="ts">
	...
	async function simpanItem() {
		...
		if (editItem) {
			await api.put(`/tugas/item/${editItem.id}`, {
				nama: fNama,
				kategori: fKategori,
				urutan: fUrutan
			});
...
<div class="space-y-4">
	<div class="flex flex-wrap items-center justify-between gap-2">
		<h1 class="text-base font-bold md:text-lg" style="color:var(--text)">Tugas Harian</h1>
	</div>
		...
		{#each [['harian', 'Checklist Hari Ini'], ['template', 'Kelola Item']] as [key, label] (key)}
			<button
				...
				style={tab === key
					...
					: 'border-color:transparent;color:var(--text-dim)'}>{label}</button
			>
		{/each}
	</div>
	...
	{#if tab === 'harian'}
			...
			<div class="flex flex-1 items-center gap-2">
				<div class="h-3 flex-1 overflow-hidden rounded-full" style="background:var(--surface2)">
					<div
						...
						style="width:{persen}%;background:var(--accent)"
					></div>
				</div>
				<span class="text-sm font-medium whitespace-nowrap" style="color:var(--text-dim)"
					>{totalSelesai}/{totalItem} ({persen}%)</span
				>
			</div>
		</div>
		...
		{:else if logRows.length === 0}
			<div class="py-12 text-center" style="color:var(--text-dim)">
				...
				{#if isManager}
					<button
						...
						style="color:var(--accent)">Tambah item di tab Kelola Item</button
					>
				{/if}
			</div>
		{:else}
	...
	{:else}
		...
		{:else}
			...
			{#if items.length === 0}
				<p class="py-8 text-center text-sm" style="color:var(--text-dim)">Belum ada item tugas.</p>
			{:else}
									...
									<td class="px-3 py-2">
										<div class="flex justify-end gap-2">
											<button
												...
												style="background:var(--surface2);color:var(--text)">Edit</button
											>
											<button
												onclick={() => (konfirmHapus = { buka: true, id: item.id })}
												class="rounded px-2 py-1 text-xs"
												style="background:#fee2e2;color:#dc2626">Hapus</button
											>
										</div>
									</td>
								</tr>
...
<SlideOver bind:open={formOpen} title={editItem ? 'Edit Item' : 'Tambah Item Tugas'}>
	{#snippet children()}
		<div class="space-y-4">
			<div>
				<label for="ft-nama" class="mb-1 block text-sm font-medium" style="color:var(--text-dim)"
					>Nama Tugas *</label
				>
				<input
					id="ft-nama"
			...
			<div>
				<label for="ft-kat" class="mb-1 block text-sm font-medium" style="color:var(--text-dim)"
					>Kategori</label
				>
				<input
					id="ft-kat"
			...
			<div>
				<label for="ft-urutan" class="mb-1 block text-sm font-medium" style="color:var(--text-dim)"
					>Urutan</label
				>
				<input
					id="ft-urutan"
			...
			<div class="flex gap-2 pt-2">
				<button
					...
					class="flex-1 rounded py-2 text-sm"
					style="background:var(--surface2);color:var(--text)">Batal</button
				>
				<button
					onclick={simpanItem}
					class="flex-1 rounded py-2 text-sm font-medium text-white"
					style="background:var(--accent)">Simpan</button
				>
			</div>
		</div>
	{/snippet}
