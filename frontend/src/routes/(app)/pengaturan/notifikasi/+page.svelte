<script lang="ts">
	...
	const TEMPLATE_DEFAULT =
		'Halo {{nama}}, kami mengingatkan bahwa piutang Anda sebesar Rp {{jumlah}} (No: {{no_transaksi}}) akan jatuh tempo pada {{jatuh_tempo}}. Mohon segera dilunasi. Terima kasih 🙏';
	...
	async function muatPiutangReminder() {
		...
		if (res.success) {
			...
			for (const p of res.data) {
				newMap[p.id] = await QRCode.toDataURL(waUrl(p), {
					width: 180,
					margin: 1,
	...
	function kirimWaAlert() {
		...
		const baris = store.alerts
			.map((a, i) => `${i + 1}. [${a.jenis.replace(/_/g, ' ')}] ${a.pesan}`)
			.join('\n');
...
<div class="space-y-6">
	...
	{#if activeTab === 'config'}
		...
		{:else if store.error}
			<p class="py-8 text-center text-sm" style="color:var(--danger)">{store.error}</p>
		{:else}
			<div class="space-y-2">
				{#each store.configs as cfg (cfg.jenis)}
					<div
						class="rounded border"
						style="background:var(--surface);border-color:{cfg.aktif
							? 'var(--accent)'
							: 'var(--border)'}"
					>
						<!-- Baris utama -->
						<div class="flex items-center gap-3 p-3">
							...
							<div class="min-w-0 flex-1">
								...
								<p class="truncate text-xs" style="color:var(--text-dim)">{cfg.deskripsi}</p>
							</div>
								...
								{#if editForm.channel === 'wa' || editForm.channel === 'keduanya'}
									<div class="space-y-1">
										<label
											for="wa-{cfg.jenis}"
											class="text-xs font-medium"
											style="color:var(--text-dim)">Nomor WhatsApp penerima</label
										>
										<input
											id="wa-{cfg.jenis}"
											type="text"
								...
								{#if JENIS_SCHEDULED.includes(cfg.jenis as any)}
									<div class="space-y-1">
										<label
											for="jam-{cfg.jenis}"
											class="text-xs font-medium"
											style="color:var(--text-dim)">Jam kirim (HH:MM)</label
										>
										<input
											id="jam-{cfg.jenis}"
											type="time"
	...
	{#if activeTab === 'log'}
		...
		{:else if store.log.length === 0}
			<p class="py-16 text-center text-sm" style="color:var(--text-dim)">
		...
		{:else}
			<div class="space-y-2">
				{#each store.log as item (item.id)}
					<div
						class="space-y-1 rounded border p-3"
						style="background:var(--surface);border-color:var(--border)"
					>
						<div class="flex items-center justify-between gap-2">
							<span
								class="rounded px-1.5 py-0.5 font-mono text-xs"
								style="background:var(--surface2);color:var(--text-dim)">{item.jenis}</span
							>
							<span
								class="text-xs font-medium"
								style="color:{STATUS_COLOR[item.status] ?? 'var(--text-dim)'}">{item.status}</span
							>
						</div>
						<p class="text-sm" style="color:var(--text)">{item.pesan}</p>
						<div class="flex justify-between text-xs" style="color:var(--text-dim)">
							<span
								>{CHANNEL_LABEL[item.channel] ?? item.channel}{item.penerima
									? ` → ${item.penerima}`
									: ''}</span
							>
	...
	{#if activeTab === 'wa'}
		<div class="space-y-4">
			...
			>
				...
				<div class="space-y-1">
					<label for="template-pesan" class="text-xs font-medium" style="color:var(--text-dim)">
						Template pesan — placeholder: <code>&#123;&#123;nama&#125;&#125;</code>
						<code>&#123;&#123;jumlah&#125;&#125;</code>
						<code>&#123;&#123;jatuh_tempo&#125;&#125;</code>
						<code>&#123;&#123;no_transaksi&#125;&#125;</code>
					</label>
					<textarea
						id="template-pesan"
						...
						class="w-full resize-none rounded border px-3 py-2 text-sm outline-none"
						style="background:var(--surface2);border-color:var(--border);color:var(--text)"
					></textarea>
				</div>
			</div>
			...
			{:else if piutangList.length === 0}
				<div
					class="rounded border p-8 text-center"
					style="background:var(--surface);border-color:var(--border)"
				>
					<p class="mb-2 text-2xl">✓</p>
					<p class="text-sm font-medium" style="color:var(--accent)">
						Tidak ada piutang jatuh tempo dalam {batasHari} hari ke depan
					</p>
				</div>
			{:else}
				<p class="text-xs" style="color:var(--text-dim)">
					{piutangList.length} piutang akan jatuh tempo dalam {batasHari} hari ke depan
				</p>
				<div class="grid gap-4 sm:grid-cols-2">
					{#each piutangList as p (p.id)}
						{@const hariSisa = p.tanggal_jatuh_tempo
							? Math.ceil((new Date(p.tanggal_jatuh_tempo).getTime() - Date.now()) / 86400000)
							: 0}
						<div
							class="space-y-3 rounded border p-4"
							style="background:var(--surface);border-color:{hariSisa <= 1
								? 'var(--danger)'
								: 'var(--warn)'}"
						>
							<!-- Info -->
							<div class="flex items-start justify-between gap-2">
								<div>
									<p class="text-sm font-bold">{p.nama_pelanggan ?? '—'}</p>
									<p class="text-xs" style="color:var(--text-dim)">{p.no_transaksi}</p>
									{#if p.kontak}
										<p class="mt-0.5 font-mono text-xs" style="color:var(--text-dim)">{p.kontak}</p>
									{:else}
										<p class="text-xs italic" style="color:var(--danger)">Nomor HP belum diisi</p>
									{/if}
								</div>
								<div class="shrink-0 text-right">
									<p class="font-mono font-bold" style="color:var(--accent)">
										Rp {fmtRp(p.sisa_piutang)}
									</p>
									<p
										class="text-xs"
										style="color:{hariSisa <= 1 ? 'var(--danger)' : 'var(--warn)'}"
									>
									...
									<p class="text-xs" style="color:var(--text-dim)">
										{p.tanggal_jatuh_tempo ? fmtTgl(p.tanggal_jatuh_tempo) : '-'}
									</p>
								</div>
							</div>
							...
							<div class="flex items-center gap-4">
								{#if qrMap[p.id]}
									<div
										class="shrink-0 rounded border p-1"
		...
		<div class="space-y-4">
			<div class="flex flex-wrap items-center justify-between gap-3">
				<p class="text-sm" style="color:var(--text-dim)">
					Cek kondisi terkini berdasarkan config yang aktif
				</p>
			...
			{:else if store.alerts.length === 0}
				...
				>
					...
					<p class="mt-1 text-xs" style="color:var(--text-dim)">
						Tidak ada alert yang perlu diperhatikan
					</p>
				</div>
			{:else}
				<div class="space-y-2">
					{#each store.alerts as alert, i (i)}
						<div
							class="rounded p-3"
							style="background:var(--surface);border:1px solid var(--border);border-left:4px solid var(--warn)"
						>
							<p class="mb-1 font-mono text-xs" style="color:var(--text-dim)">{alert.jenis}</p>
