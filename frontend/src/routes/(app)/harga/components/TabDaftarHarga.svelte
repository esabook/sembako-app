			...
			{#each pagedList as b (b.id)}
				<tr class="border-t" style="border-color:var(--border)">
					...
					{#if !hidden.has('harga_beli_terakhir')}
						<td class="px-3 py-2 text-right font-mono text-xs" style="color:var(--text-dim)">
							{rp(b.harga_beli_terakhir)}
						</td>
					{/if}
					{#if !hidden.has('harga_jual_eceran')}
						<td class="px-3 py-2 text-right font-mono text-xs font-bold" style="color:var(--text)">
							{rp(b.harga_jual_eceran)}
						</td>
					{/if}
					{#if !hidden.has('margin_eceran')}
						<td class="px-3 py-2 text-right font-mono text-xs" style={marginColor(b.margin_eceran)}>
							{pct(b.margin_eceran)}
						</td>
					{/if}
					...
					{#if !hidden.has('margin_grosir')}
						<td class="px-3 py-2 text-right font-mono text-xs" style={marginColor(b.margin_grosir)}>
							{pct(b.margin_grosir)}
						</td>
					{/if}
					{#if !hidden.has('aksi')}
						<td class="px-3 py-2">
							<div class="flex justify-end gap-1">
								...
								<button
									...
									style="color:var(--text-dim)">Histori</button
								>
							</div>
						</td>
					{/if}
