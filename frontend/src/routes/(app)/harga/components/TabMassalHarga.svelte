...
<div class="space-y-4">
	...
	<div class="rounded border" style="border-color:var(--border)">
		...
		>
			<input
				type="checkbox"
				checked={store.massalChecked.size === store.barangList.length &&
					store.barangList.length > 0}
		...
		<div class="max-h-64 overflow-y-auto">
			{#each store.barangList as b (b.id)}
				<label
					class="flex cursor-pointer items-center gap-3 border-b px-3 py-2 hover:opacity-80"
				...
				>
					...
					<div class="min-w-0 flex-1">
						<div class="truncate text-xs font-medium" style="color:var(--text)">
							{b.nama_barang}
						</div>
					...
					{#each store.massalPreview as p (p.id)}
						<tr class="border-t" style="border-color:var(--border)">
							...
							<td class="px-3 py-2 text-right font-mono" style="color:var(--text-dim)"
								>{rp(p.harga_eceran_lama)}</td
							>
							<td class="px-3 py-2 text-right font-mono font-bold" style="color:var(--accent)"
								>{rp(p.harga_eceran_baru)}</td
							>
							<td class="px-3 py-2 text-right font-mono" style="color:var(--text-dim)"
								>{rp(p.harga_grosir_lama)}</td
							>
							<td class="px-3 py-2 text-right font-mono font-bold" style="color:var(--accent)"
								>{rp(p.harga_grosir_baru)}</td
							>
