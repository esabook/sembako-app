...
<div class="min-h-screen flex flex-col" style="background:var(--bg);color:var(--text);font-family:'JetBrains Mono',monospace">

  <!-- Header: jam & tanggal -->
  <div class="flex items-center justify-between px-6 py-3 border-b" style="border-color:var(--border)">
    <div class="text-sm font-bold tracking-wider" style="color:var(--text-dim)">ABSENSI KARYAWAN</div>
    ...
    {#if phase === 'select'}
      <div class="flex flex-col items-center gap-4 w-full max-w-lg">
        ...
        {#if karyawanList.length === 0}
          <p class="text-xs" style="color:var(--text-dim)">Belum ada karyawan dengan PIN. Atur PIN di halaman Karyawan.</p>
        {:else}
          <div class="grid grid-cols-2 gap-3 w-full">
            {#each karyawanList as k (k.id)}
              <button
                onclick={() => pilihKaryawan(k)}
                class="py-5 px-4 rounded-xl font-bold text-base transition-all active:scale-95 text-center"
    ...
    {:else if phase === 'pin' || (phase === 'loading' && digits === '')}
      <div class="flex flex-col items-center gap-5">
        <div class="text-center">
          <p class="text-xs mb-1" style="color:var(--text-dim)">Masuk PIN untuk</p>
        ...
        <button onclick={() => reset()} class="text-xs mt-1" style="color:var(--text-dim)">← Kembali pilih nama</button>
      </div>

    <!-- ── LOADING ────────────────────────────────────────────────────── -->
    {:else if phase === 'loading'}
      <p class="text-sm animate-pulse" style="color:var(--text-dim)">Memproses...</p>
    ...
    {:else if phase === 'confirm'}
      <div class="flex flex-col items-center gap-5 text-center">
        <div class="rounded-full flex items-center justify-center font-bold text-2xl"
          ...
          {selected?.nama.trim().split(/\s+/).slice(0,2).map(w=>w[0]).join('').toUpperCase()}
        </div>
        <div>
          <p class="text-2xl font-bold">{selected?.nama}</p>
        ...
        <div class="flex gap-3">
          {#if statusHariIni === 'belum'}
            <button onclick={doMasuk}
              class="px-10 py-4 rounded-xl font-bold text-lg active:scale-95"
          ...
          {:else}
            <div class="px-10 py-4 rounded-xl font-bold text-sm"
              style="background:var(--surface2);color:var(--text-dim)">Sudah selesai hari ini</div>
          {/if}
          <button onclick={() => reset()} class="px-5 py-4 rounded-xl text-sm"
